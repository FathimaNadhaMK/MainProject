export class ResourceFetcher {
        ...
        try {
            ...
            if (data.error) {
                ...
                if (message.includes("not been used") || message.includes("disabled")) {
                    console.warn("[ResourceFetcher] YouTube API is disabled or not configured. Using fallback.");
                } else {
                    console.error("[ResourceFetcher] YouTube API Error:", message, "- Using fallback");
                }
    ...
    async fetchFreeResources(topic) {
        // We now fetch from APIs (Dev.to, Stack Overflow, Medium RSS) based on topic instead of static lists
        const devTo = await this.fetchDevToArticles(topic);
        const stackOverflow = await this.fetchStackOverflowQA(topic);
        const medium = await this.fetchMediumArticles(topic);

        // Fallback for general official docs if specific query fails to bring results
        const topicLower = topic.toLowerCase();
        let docs = [];
        if (topicLower.includes("web") || topicLower.includes("js") || topicLower.includes("html") || topicLower.includes("css") || topicLower.includes("react")) {
            docs.push({
        ...
        return [...devTo, ...stackOverflow, ...medium, ...docs].slice(0, 6);
    }

    async fetchDevToArticles(topic) {
        const query = encodeURIComponent(topic.replace(/^(Task title|Implementation|Learning):?\s*/i, '').trim());
        const apiKey = process.env.DEVTO_API_KEY || 'xvGtgcAh159qYCRyJavQUb2r'; // User provided or env
        try {
            const res = await fetch(`https://dev.to/api/articles?search=${query}&per_page=3`, {
            ...
            console.error("[ResourceFetcher] Dev.to error:", e.message);
            return [];
        }
    }

    async fetchStackOverflowQA(topic) {
        const query = encodeURIComponent(topic.replace(/^(Task title|Implementation|Learning):?\s*/i, '').trim());
        const apiKey = process.env.STACKEXCHANGE_API_KEY || 'rl_NTGSYacLofD1aNyEW1aY6LUc8'; // User provided or env
        try {
            const url = `https://api.stackexchange.com/2.3/search/advanced?order=desc&sort=relevance&q=${query}&site=stackoverflow&key=${apiKey}&filter=withbody`;
            const res = await fetch(url);
            if (!res.ok) return [];
            const data = await res.json();
            ...
            console.error("[ResourceFetcher] StackOverflow error:", e.message);
            return [];
        }
    }

    async fetchMediumArticles(topic) {
        // Tag search based on main topic word
        const cleanTopic = topic.replace(/[^a-zA-Z0-9\s]/g, '').trim().split(' ')[0].toLowerCase();
        const tag = cleanTopic.substring(0, 20); // Keep it small
        if (!tag) return [];
        try {
            const res = await fetch(`https://api.rss2json.com/v1/api.json?rss_url=https://medium.com/feed/tag/${tag}`);
            if (!res.ok) return [];
            const data = await res.json();
            return (data.items || []).slice(0, 3).map(item => ({
                platform: "Medium",
            ...
            console.error("[ResourceFetcher] Medium RSS error:", e.message);
            return [];
        }
    }
