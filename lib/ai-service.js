...
import { z } from "zod";
import { withRetry, withCache } from "./ai-retry";
...
"interviewTimeline": { "dsaStart": "week X", "systemDesignStart": "week X", "behavioralStart": "week X", "readyToApply": "week X" }
    ...
    try {
      // Use cache for roadmaps as they are identical for the same user/preferences
      const cacheKey = `roadmap_${user.id}_${JSON.stringify(preferences)}`;
