...
class AIService {
  ...
  parseAIResponse(text) {
    try {
      // 1. Try direct parsing after cleaning markdown code blocks
      const cleanText = text.replace(/```json\n?|\n?```/g, "").trim();
      return JSON.parse(cleanText);
    } catch (error) {
      console.warn("AI response parse error:", error.message);
      // 2. Try secondary parse by forcing JSON extraction and stripping invalid characters/trailing commas
      try {
...
Return ONLY valid JSON with this structure:
{
  ...
  "nextSteps": ["suggested next steps"]
    ...
    try {
      ...
      // Ensure the structure is correct even if AI misses some keys
      return {
        score: data.score || 0,
        summary: data.summary || "Project evaluation complete.",
        strengths: data.strengths || [],
        weaknesses: data.weaknesses || [],
        feedback: data.feedback || "Good progress on the project.",
